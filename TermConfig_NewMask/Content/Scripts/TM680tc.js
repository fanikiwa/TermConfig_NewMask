...
var selectedGrid = 0;
var gobackhide = 0;
var readerStaticId = 0;
var levelCaption = "";

$(function () {
    try {

        $("#btnDeleteReader").prop("disabled", false);
        ...
        //$("#btnDeleteReader").click(function (e) {
        //    e.preventDefault();
        //    //var index = grdTerminalReader.GetFocusedRowIndex();
        //    //getLocalizedText("TerminalInstanceDeleteWarning");
        //    ConfirmDeleteReaderDialog();
        //});
        ...
        //else {
        //    getLocalizedText("TerminalInstanceDeleteWarning");
        //    ConfirmDeleteDialog(levelCaption);
        //}
        //});
        ...
        $('#btnDeleteReader').click(function (evt) {
            ...
            //ConfirmDeleteReaderDialog(levelCaption);
            //ConfirmDeleteReaderDialog(levelCaption);
            var externalreaderid = document.getElementById('lblExId').value;
            console.log(externalreaderid);
            if (externalreaderid === "" || externalreaderid === "0") {
                getLocalizedText("noSelection");
                alert(levelCaption);
            }
            else {
        ...
        $("#btnSendMasterData").click(function (evt) {
            evt.preventDefault();
        });

        $("#btnGetBookings").click(function (evt) {
            evt.preventDefault();
        });

        $("#btnSetSystemTime").click(function (evt) {
            evt.preventDefault();
        });

        $("#btnTestConnection").click(function (evt) {
            evt.preventDefault();
        });

        $("#btnApplyReaderStatic").click(function (e) {
            e.preventDefault();
            FillGridValues();
        });

        $("#btnBackReaderStatic").click(function (e) {
            e.preventDefault();
            $(".secpart1").show();
            $(".secpart2").hide();
            $(".secGridview").hide();
        ...
        //$("#btnDelete").click(function (evt) {
        //    evt.preventDefault();
        //    if ($("#txtTerminalId").val() === "" || $("#txtTerminalId").val() === "0") {
        //        getLocalizedText("noSelection");
        //        alert(levelCaption);
        //    } else {
        //        getLocalizedText("TerminalInstanceDeleteWarning");
...
function ConfirmDeleteDialog(message) {
    try {
        getLocalizedText("deleteprompttm680tc");
        var message = levelCaption;
        ...
        getLocalizedText("acceptdeletetm680tc");
        $("#btnOk").text(levelCaption);
        getLocalizedText("no");
        $("#btnNoDelete").text(levelCaption);
        getLocalizedText("canceldeletetm680tc");
...
function getLocalizedText(key) {
    try {
        ...
        $.ajax({
            type: "POST",
            async: false,
            url: "TM680tc.aspx/GetLocalizedText",
    ...
    try {
        ...
        if (response !== null) {
            ...
            if (response.ID !== null && response.ID !== 0) {
                ...
                ShowUserZkTerminals();
                grdTerminalReader.PerformCallback(response.ID);
...
function DeleteTerminalReader() {
    document.getElementById('confirmDelete').innerHTML = "";
    $("#btnAddReader").prop("disabled", false);
    if ($("#ddlCurrentZkTerminalInstance").val() === null) {
        $("#lblExId").text("");
        $("#lblExInfo").text("");
        $("#lblExLaserType").text("");
        $("#txtExDescription").val("");
...
function DeleteCurrentZkTerminalInstance_Callback(response) {
    if (response === undefined || response === null || response.length === 0) {
        ...
        if (response.HasAPPosting === true) {
            $('#chkUseAccessControl').prop('checked', true);
        } else {
            $('#chkUseAccessControl').prop('checked', false);
        }
...
function save() {
    ...
    var chkbackToReadFingerPrint = false;
    var chkAPPosting = false;
    if ($('#chkUseAccessControl')[0].checked === true) {
        chkAPPosting = true;
    }
    else if ($('#chkUseAccessControl')[0].checked === false) {
        chkAPPosting = false;
    }
    var chkProfFirmware = false;
    if ($('#chkProfFirmware')[0].checked = true) {
        chkProfFirmware = true;
    }
    else if ($('#chkProfFirmware')[0].checked = false) {
        chkProfFirmware = false;
    }
    var chkpersonnelNumberPin = false;
    if ($('#chkpersonnelNumberPin')[0].checked = true) {
        chkpersonnelNumberPin = true;
    }
    else if ($('#chkpersonnelNumberPin')[0].checked = false) {
        chkpersonnelNumberPin = false;
    }
...
ddlFunctionKey1, ddlFunctionKey2, ddlFunctionKey3, ddlFunctionKey4, ddlFunctionKey5, ddlFunctionKey6, ddlFunctionKey7, ddlFunctionKey8, IsActiveTerminal, txtRelaySwitching, chkpersonnelNumberPin, readerInternDesc, readerExternDesc, readerInternDirec, readerExternDirec, readerRelayTime, externalReaderId, reader_Type, readerStaticId, OnSave_Callback, OnSaveFail_Callback);
    } else {
        PageMethods.UpdateZkTerminalInstance(ddlCurrentZkTerminalInstance, ddlTerminalType, selectedTerminalID, txtUserTerminalId, txtDescription, txtSerialNumber, ddlLinkType,
            txtIpAddress, txtIpPort, chkbackToReadFingerPrint, chkAPPosting, chkProfFirmware, txtInfoText1, txtInfoText2, txtInfoText3, txtInfoText4,
ddlFunctionKey1, ddlFunctionKey2, ddlFunctionKey3, ddlFunctionKey4, ddlFunctionKey5, ddlFunctionKey6, ddlFunctionKey7, ddlFunctionKey8, IsActiveTerminal, txtRelaySwitching, chkpersonnelNumberPin, readerInternDesc, readerExternDesc, readerInternDirec, readerExternDirec, readerRelayTime, externalReaderId, internReaderId, reader_Type, readerStaticId, OnEdit_Callback, onEditFail_Callback);
    }
}
...
function DeleteReader() {
    resetConfirmationDiv();
    var index = grdTerminalReader.GetFocusedRowIndex();
    var key = grdTerminalReader.GetRowKey(index);
    PageMethods.DeleteReaderInstance(key, OnDeleteReader_Callback);
}
function OnDeleteReader_Callback() {
    grdTerminalReader.PerformCallback(0);
}
function ConfirmDeleteReaderInstance(message) {
    var box_content = '<div id="overlay"><div id="box_flame"><div id="dialogBox">  <img src="../../Images/FormImages/stop-save1-01.png" alt="Stop" class="stopPic" height="150" width="150" align="middle"> <br/>' + message + '<br/> <button id="btnOk"  onclick="DeleteReader()"></button><button id="btnNo"  onclick="CancelDelete()"></button><button id="btnCancel"  onclick="CancelDelete()"></button></div></div></div>';
    document.getElementById('confirmDelete').innerHTML = box_content;
    getLocalizedText("yes");
    $('#btnOk').text(levelCaption);
    getLocalizedText("no");
...
function SetNewInternReader() {
    ...
    $("#lblTILaserType").text("RFID");
    $("#txtTIRelayTime").val("5");
    $("#lblTIStatus").text("Inaktiv");
    $("#lblTIStatus").css("color", "red");
    $("#ddlExDirection").val("0").attr("selected", "selected");
}
function DisableExternalReaderControls() {

    $("#btnDeleteReader").prop("disabled", true);
    $("#txtExDescription").prop("readonly", true);
}
function EnableExternalReaderControls() {

    $("#btnDeleteReader").prop("disabled", false);
    $("#txtExDescription").prop("readonly", false);
}

function showTerminalsInfo() {
    $(".secpart1").hide();
    $(".secpart2").hide();
    $(".secpart3, .secGrid1, .secBottomLeft, .bttmsecRightbtns").hide();
    $(".secGridview").show();
    $(".secGridviewsplit2").show();
    $(".secgridhJDNew").show();
    $(".secAcceptChanges").show();
    $(".bttmsec5Left, .bttmsec5Left").show();

    grdReaderStatic.PerformCallback();
    grdReaderInfo.PerformCallback();
}

function grdReaderStaticRowClick(s, e) {
    selectedIndex = e.visibleIndex;
    selectedGrid = 1;
    //var index = e.visibleIndex;
    //window.grdReaderStatic.GetRowValues(index, "ReaderType", GetSelectedExternalReader);
    //EnableExternalControls();
    var grdReaderInfoIndex = grdReaderInfo.GetFocusedRowIndex();
    if (grdReaderInfoIndex !== -1) {
        grdReaderInfo.GetRow(grdReaderInfo.GetFocusedRowIndex()).className = "dxgvDataRow_Aqua"
    }
}
function grdReaderStaticRowDoubleClick(s, e) {
    selectedIndex = e.visibleIndex;
    selectedGrid = 1;
    //var index = e.visibleIndex;
    //window.grdReaderStatic.GetRowValues(index, "ReaderType", GetSelectedExternalReader);
...
function GetSelectedExternalReader(value) {
    $("#lblExId").text("2");
    $("#lblExInfo").text("Extern");
    readerStaticId = value[0];
    $("#lblExLaserType").text(value[1]);
}
function grdReaderInfoRowClick(s, e) {
    selectedIndex = e.visibleIndex;
    selectedGrid = 2;
    //var index = e.visibleIndex;
    //window.grdReaderInfo.GetRowValues(index, "ReaderType", GetSelectedExternalReader);
    //EnableExternalControls();
    var grdReaderStaticIndex = grdReaderStatic.GetFocusedRowIndex();
    if (grdReaderStaticIndex !== -1) {
        grdReaderStatic.GetRow(grdReaderStatic.GetFocusedRowIndex()).className = "dxgvDataRow_Aqua"
    }
}
function grdReaderInfoRowDoubleClick(s, e) {
    selectedIndex = e.visibleIndex;
    selectedGrid = 2;
    //var index = e.visibleIndex;
    //window.grdReaderInfo.GetRowValues(index, "ReaderType", GetSelectedExternalReader);
...
function EnableExternalControls() {
    $("#btnAddReader").prop("disabled", true);
    EnableExternalReaderControls();
    $("#ddlExDirection").val("0").attr("selected", "selected");
}
function FillGridValues() {
    if (selectedIndex !== -1 && selectedGrid !== 0) {
        switch (selectedGrid) {
            case 1:
                window.grdReaderStatic.GetRowValues(selectedIndex, "ID;ReaderType", GetSelectedExternalReader);
                EnableExternalControls();
                break;
            case 2:
                window.grdReaderInfo.GetRowValues(selectedIndex, "ID;ReaderType", GetSelectedExternalReader);
                EnableExternalControls();
                break;
        }
    }
