...
function getLocalizedText(key) {
    try {
        ...
        $.ajax({
            type: "POST",
            async: false,
            url: "TM900bc.aspx/GetLocalizedText",
...
function DeleteCurrentZkTerminalInstance_Callback(response) {
    if (response === undefined || response === null || response.length === 0) {
        ...
        if (response[0].ConnectionType === "keine") {
            $("#cboLinkType option").remove();
            $("#cboLinkType").append($('<option></option>').val(0).html("keine"));
            $("#cboLinkType").append($('<option></option>').val(1).html("TCP/IP"));
            $("#cboLinkType").append($('<option></option>').val(2).html("USB"));
            $("#cboLinkType").val("0").attr("selected", "selected");
        }
        if (response[0].ConnectionType === "None") {
            $("#cboLinkType option").remove();
            $("#cboLinkType").append($('<option></option>').val(0).html("None"));
            $("#cboLinkType").append($('<option></option>').val(1).html("TCP/IP"));
            $("#cboLinkType").append($('<option></option>').val(2).html("USB"));
            $("#cboLinkType").val("0").attr("selected", "selected");
        }
        if (response[0].ConnectionType === "TCP/IP") {
            ...
            $("#cboLinkType").append($('<option></option>').val(1).html("TCP/IP"));
            $("#cboLinkType").append($('<option></option>').val(2).html("USB"));
            $("#cboLinkType").val("1").attr("selected", "selected");
        }
        if (response[0].ConnectionType === "USB") {
            ...
            $("#cboLinkType").append($('<option></option>').val(1).html("TCP/IP"));
            $("#cboLinkType").append($('<option></option>').val(2).html("USB"));
            $("#cboLinkType").val("2").attr("selected", "selected");
        }
...
function save() {
    ...
    var cboLinkType = $("#cboLinkType option:selected")[0] === null ? "TCP/IP" : $("#cboLinkType option:selected")[0].innerHTML;
    ...
    var chkbackToReadFingerPrint = $('#chkFingerPrint')[0].checked;
    var chkAPPosting = $('#chkUseAccessControl')[0].checked;
    var chkProfFirmware = $('#chkProfFirmware')[0].checked;
    var chkpersonnelNumberPin = $('#chkPInPers')[0].checked;
    ...
    if (ddlCurrentZkTerminalInstance === "" || ddlCurrentZkTerminalInstance === null || ddlCurrentZkTerminalInstance === undefined) {
        ...
        PageMethods.CreateZkTerminalInstance(ddlTerminalType, selectedTerminalID, txtUserTerminalId, txtDescription, txtSerialNumber, cboLinkType,
...
ddlFunctionKey1, ddlFunctionKey2, ddlFunctionKey3, ddlFunctionKey4, ddlFunctionKey5, ddlFunctionKey6, ddlFunctionKey7, ddlFunctionKey8, IsActiveTerminal, txtRelaySwitching, chkpersonnelNumberPin, readerInternDesc, readerInternDesc, readerInternDirec, readerInternDirec, readerRelayTime, OnSave_Callback, OnSaveFail_Callback);
    } else {
        PageMethods.UpdateZkTerminalInstance(ddlCurrentZkTerminalInstance, ddlTerminalType, selectedTerminalID, txtUserTerminalId, txtDescription, txtSerialNumber, cboLinkType,
            txtIpAddress, txtIpPort, chkbackToReadFingerPrint, chkAPPosting, chkProfFirmware, txtInfoText1, txtInfoText2, txtInfoText3, txtInfoText4,
ddlFunctionKey1, ddlFunctionKey2, ddlFunctionKey3, ddlFunctionKey4, ddlFunctionKey5, ddlFunctionKey6, ddlFunctionKey7, ddlFunctionKey8, IsActiveTerminal, txtRelaySwitching, chkpersonnelNumberPin, readerInternDesc, readerInternDesc, readerInternDirec, readerInternDirec, readerRelayTime, internReaderId, OnEdit_Callback, onEditFail_Callback);
    }
}
...
function OnSave_Callback(response) {
    ...
    if (response.HasFPRead === true) {
        $('#chkFingerPrint').prop('checked', true);
    } else {
        $('#chkFingerPrint').prop('checked', false);
    }
    ...
    if (response.HasPersNoPin === true) {
        $('#chkPInPers').prop('checked', true);
    } else {
        $('#chkPInPers').prop('checked', false);
    }
    if (response.HasProfFirmware === true) {
        $('#chkProfFirmware').prop('checked', true);
    } else {
...
function OnEdit_Callback(response) {
    ...
    $("#txtTIRelayTime").val(response.relayTime);
    $("#lblExId").text("1a");
    //$("#lblExInfo").text(response.readerTypeIntern_2);
    $("#lblExLaserType").text(response.readerTypeIntern_2);
    //$("#txtExDescription").val(response.externReaderDesc); 
    //$("#ddlExDirection").val(response.externDirec).attr("selected", "selected");
